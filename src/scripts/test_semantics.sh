...
TESTS_DIR="$PROJECT_DIR/src/tests/lexer_yacc"
...
echo -e "${CYAN}========================================${RESET}"
echo -e "${CYAN}  Verificando o Analisador Semântico...${RESET}"
echo -e "${CYAN}========================================${RESET}"
...
if [ ! -f "$EXE" ]; then
    ...
    make all
    if [ $? -ne 0 ]; then
        echo -e "${RED}Erro ao compilar o compilador!${RESET}"
        exit 1
    fi
fi
...
# Executar testes
echo -e "${CYAN}========================================${RESET}"
echo -e "${CYAN}  Executando testes semânticos (Meta 3)...${RESET}"
echo -e "${CYAN}========================================${RESET}"
echo ""
...
for java_file in $JAVA_FILES; do
    ...
    # Flag rigorosa para a Meta 3 (Semântica)
    flag="-s"
...
echo ""
echo -e "${CYAN}========================================${RESET}"
echo -e "${CYAN}  Resumo Meta 3 (Semântica)${RESET}"
echo -e "${CYAN}========================================${RESET}"
echo -e "  Total:   $total"
